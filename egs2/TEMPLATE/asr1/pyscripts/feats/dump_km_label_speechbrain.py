...
def dump_label(
    ...
    **kwargs
    ...
    # init speechbrain discrete ssl model
    #used_ssl_layer_num = [1,3,7,12,18,23]
    model_hub = "microsoft/wavlm-large"
    save_path = "/home/clin/links/store2/data_generation/savedir"
    ssl_layer_num = [23]
    deduplicate =[False for _ in ssl_layer_num]  # do not deduplicate semantic tokens
    bpe_tokenizers=[None for _ in ssl_layer_num]  # do not use BPE tokenizers
    vocoder_repo_id = "speechbrain/hifigan-wavlm-k1000-LibriTTS"
    kmeans_dataset = "LibriSpeech"
    num_clusters = [1000 for _ in ssl_layer_num]  # use 1000 clusters for all layers
    ssl_model = WavLM(model_hub, save_path, output_all_hiddens=True)
    speech_encoder = DiscreteSSL(save_path, ssl_model, vocoder_repo_id=vocoder_repo_id, kmeans_dataset=kmeans_dataset,num_clusters=num_clusters, layers_num=ssl_layer_num, device="cuda").cuda()
    speech_encoder.codec_vocoder.to("cuda")  # ensure the vocoder is on the same device
    speech_encoder.eval()

    speech_encoder_forward_config = {
        "SSL_layers": ssl_layer_num,
        "deduplicates": deduplicate,
        "bpe_tokenizers": bpe_tokenizers
    }
    oneline_feature_extract = True
    ...
    if not online_feature_extract:
        ...
        with file_writer_helper(
            ...
            for utt_ids, data in iterator:
                normalized_audio_length  = data["speech_lengths"] / data["speech_lengths"].max()
                tokens, _, _ = speech_encoder.encode(
                    data["speech"].to(speech_encoder.device),
                    wav_lens=normalized_audio_length.to(speech_encoder.device),
                    **speech_encoder_forward_config
                )
                tokens = tokens.squeeze(-1).cpu().numpy()
                for idx, utt in enumerate(utt_ids):
                    writer[utt] = tokens[idx]
                """
